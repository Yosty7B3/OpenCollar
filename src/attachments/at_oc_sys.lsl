...
string g_sAttachmentName="";
string g_sDevStage="";
string g_sCollarVersion="7.5.0"; // in reality: 7.4.1 but since checkversion in this and previous code does not allow for a minor rev number, this is what we have to work with
/*
...
list g_lMainMenu = ["~", "~", "-", "-", "-", "-"];
MainMenuButton(string sSubMenu, integer index){
    if(!~index){
        index = llListFindList(g_lMainMenu, [sSubMenu]);        
        if(!~index){ // good, doesn't exist yet
           g_lMainMenu += [sSubMenu]; 
           return;
        }        
    }
    g_lMainMenu = llListReplaceList(g_lMainMenu, [sSubMenu], index, index);
}    
...
Debug(list ITEMS){
    ...
    string final;
    for(i=0;i<end;i++){ final+=llList2String(ITEMS,i)+" "; }
...
/*    list lStaticButtons=["Apps"];
    ...
    */
   // g_lMainMenu = ["Apps"] + g_lMainMenu;
    
    if (g_iLocked)  MainMenuButton("UNLOCK", 0);
    else MainMenuButton("LOCK", 0);
    MainMenuButton("Apps", 1)  ;                
                 
    list lStaticButtons;             
    integer l = 9-llGetListLength(g_lMainMenu);    
    while(l<3 && l>0){ 
        lStaticButtons += ["-"]; // fill the remaining buttons to keep things looking clean
        l--;
    }
    
    lStaticButtons = g_lMainMenu+lStaticButtons+["Access","Settings","Help/About"];
...
default {
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
          //  Debug(["link_message( ",iSender, iNum, sStr, kID, " )"]);
        ...
        if (iNum == MENUNAME_RESPONSE) {
            ...
            //Debug(["link_message( ",iSender, iNum, sStr, kID, " ) MainMenuButton(", sSubMenu, 3, ")"]); 
            if (sName=="AddOns" || sName=="Apps"){  //we only accept buttons for apps nemu
                ...
                if (sSubMenu=="Animations")             MainMenuButton("Animations", 2);
                else if (sSubMenu==g_sAttachmentName)   MainMenuButton(sSubMenu, 3);   
                else if (sSubMenu == "Leash")           MainMenuButton("Leash", 4); 
                else if (sSubMenu=="RLV")               MainMenuButton("RLV", 5); 
                else if (llStringLength(g_sAttachmentName)==0) RebuildMenu(0, "", 0); // try again...
                else MainMenuButton(sSubMenu, -1);                
            ...
            //else if (sToken == "intern_dist") g_sOtherDist = sValue;  //did this have a purpose?
            else if (sStr == "settings=sent") {
                llOwnerSay("settings loaded..."); // todo: remove?
            /* disable capture
            } else if(sToken == "capture_isActive"){
                g_iCaptureIsActive=TRUE;//*/
            } else if(sToken == "attachment_name"){ 
                g_sAttachmentName = sValue; 
            }
        /* disable capture    
