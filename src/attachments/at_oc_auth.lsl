...
string g_sScriptVersion = "7.5.0";

//*
integer LINK_CMD_DEBUG=1999;
DebugOutput(key kID, list ITEMS){
    if(kID==NULL_KEY) kID = llGetOwner();
    integer i=0;
...
integer g_iIsAttachment;   
integer g_iCollarIsWorn;         

list g_lCommandQueue;


ExecuteCommandsFor(key kID, integer iAuth){
    kID = llGetOwnerKey(kID);
    integer index = llListFindList(g_lCommandQueue, [kID]);   
    while(~index){
        string sStr = llList2String(g_lCommandQueue, index+1 );
        llMessageLinked(LINK_SET, iAuth, sStr, kID);          
        g_lCommandQueue = llDeleteSubList(g_lCommandQueue, index, index+1); 
        index = llListFindList(g_lCommandQueue, [kID]);  
    }  
}
...
// ~ is bitwise NOT which is used for the llListFindList function to simply turn the result "-1" for "not found" into a 0 (FALSE)
    if (! ~llListFindList(g_lOwner,[(string)kCmdr])){
...
AddUniquePerson(string sPersonID, string sToken, key kID) {
    list lPeople;
    if ( ! ~llListFindList(g_lOwner,[(string)kID]) )
...
default {
    ...
    /* for Attachments only      
            ...
            index = CommandIndex(kID)
            if(~index)
        key kAuth = llList2Key(g_lCommandQueue, index);            
        integer iAuth = Auth(kAuth);                             
        if (iAuth >= CMD_OWNER && iAuth <= CMD_EVERYONE) { 
            if(CollarAuth < iAuth) // which has the best auth value? attachement's own stored auth or collar's auth (lower is better)
                iAuth = CollarAuth;                        
            
        Debug(["Authed as "+(string)iAuth]);    
        ExecuteCommandsFor(kAuth, iAuth);
    //*/
        //Indicator(FALSE);
        llSetTimerEvent(0);
        //Debug("timer ");  
    ...
    listen(integer iChan, string sSpeaker, key kID, string sMsg) { 
        if(iChan == g_iInterfaceChannel && llGetOwnerKey(kID) == g_sWearerID){
            if (sMsg == "OpenCollar=Yes")  g_iCollarIsWorn = TRUE;  // looks like wearer is wearing a collar                        
            else if(llGetSubString(sMsg, 0, 8) == "AuthReply"){ 
                list lReplyParams = llParseString2List(sMsg, ["|"], []);
                key kAuth = llList2String(lReplyParams, 1);
                integer CollarAuth = llList2Integer(lReplyParams, 2);    
                integer iAuth = Auth(kAuth);                             
                if (iAuth >= CMD_OWNER && iAuth <= CMD_EVERYONE) { 
                    if(CollarAuth < iAuth) // which has the best auth value? attachement's own stored auth or collar's auth (lower is better)
                        iAuth = CollarAuth;                        
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
           //Debug(["link_message(", iNum, sStr, kID, " )"]);
        if (iNum == CMD_ZERO) { //authenticate messages on CMD_ZERO
            ...
            if ( kID == g_sWearerID && sStr == "runaway") {   // note that this will work *even* if the wearer is blacklisted or locked out
                ...
                UserCommand(iAuth, "runaway", kID, FALSE);
            //*  Attachments only code 
            else if (g_iIsAttachment && g_iCollarIsWorn && iAuth!=CMD_OWNER && iAuth!=CMD_BLOCKED){ // if not owner or blocked    

                g_lCommandQueue += [kID, sStr];                
                llRegionSayTo(g_sWearerID, 0, "AuthRequest|"+(string)kID);   
                llRegionSayTo(g_sWearerID, g_iInterfaceChannel, "AuthRequest|"+(string)kID);        // send an AuthRequest to the collar    
                llResetTime();llSetTimerEvent(10);  // timeout for if collar was detached or to laggy to respond
            }//*/    
            ...
            //* for Attachments only
            } else if(llToLower(llGetSubString(sToken,0,i)) == "attachment_"){ 
                if(llGetSubString(sToken,i+1,-1) == "name"){  
                    g_iIsAttachment = TRUE;
                    
                    if(!g_iInterfaceChannel){ 
                        g_iInterfaceChannel = (integer)("0x" + llGetSubString(g_sWearerID,30,-1));
                        if (g_iInterfaceChannel > 0) g_iInterfaceChannel = -g_iInterfaceChannel; 
                        llListen(g_iInterfaceChannel, "", "", "");                        
                        //DebugOutput(g_sWearerID, ["open InterfaceChannel", g_iInterfaceChannel]); 
                    }                         
                    llRegionSayTo(g_sWearerID, g_iInterfaceChannel, "OpenCollar?");                 
                }
            // Attacement end   */
            }
...
/*        } else if (iNum == LOADPIN && sStr == llGetScriptName()) {
            ...
            if(sStr == "ver")return; // basically this command was: <prefix> versions
