...
default {
    ...
    state_entry() {
       ...
       // llSetMemoryLimit(49152);  //2015-05-06 (6180 bytes free)
        ...
        if ((iAttachPt > 0 && iAttachPt < 31) || iAttachPt == 39) { // if collar is attached to the body (thus excluding HUD and root/avatar center)
            ...
            /* Attachements don't need the highlander test
 ...
 //           llRegionSayTo(g_kWearer, g_iInterfaceChannel, "Attachement="+g_sAttachmentName);
            //*/
    }
...
/*/              
                if (sMsg == "OpenCollar?") { llRegionSayTo(g_kWearer, g_iInterfaceChannel, "Attachement="+g_sAttachmentName);                
//*/
        ...
        if (iChan == g_iInterfaceChannel) {
            ...
            if (sMsg == "OpenCollar?") llRegionSayTo(g_kWearer, g_iInterfaceChannel, "OpenCollar=Yes");
                ...
                llRequestPermissions(g_kWearer,PERMISSION_ATTACH);
            /*/
            if (sMsg == "OpenCollar?") { llRegionSayTo(g_kWearer, g_iInterfaceChannel, "Attachment="+g_sAttachmentName);
            //*/
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID) {
        if (iNum >= CMD_OWNER && iNum <= CMD_EVERYONE) UserCommand(kID, iNum, sStr);
            ...
            /* disable highlander check     
                ...
                llRegionSayTo(g_kWearer, g_iInterfaceChannel, "Attachment="+sValue);                
            }
        } else if(iNum == -99999){
        ...
        /*    
           ...
           DebugOutput(NULL_KEY,["link_message(", iNum, sStr, kID, " )"]);
